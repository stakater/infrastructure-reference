...
data "template_file" "worker-filebeat-additional-user-data" {
  ...
  vars {
    qa_config_bucket="${module.config-bucket.bucket_name}"
  }
}
...
resource "aws_security_group" "worker-sg-elb" {
  name_prefix = "${var.stack_name}-qa-worker-elb-"
  vpc_id      = "${module.network.vpc_id}"

  tags {
    Name        = "${var.stack_name}-qa-worker-elb"
    managed_by  = "Stakater"
  }
...
resource "aws_elb" "worker-elb-internal" {
  name                      = "${var.stack_name}-qa-wrkr-int"
  security_groups           = ["${aws_security_group.worker-sg-elb.id}"]
  ...
  tags {
    Name        = "${var.stack_name}-qa-worker-internal"
    managed_by  = "Stakater"
  }
