...
resource "aws_security_group" "mysql-sg-elb" {
  name_prefix = "${var.stack_name}-dev-mysql-elb-"
  vpc_id      = "${module.network.vpc_id}"

  tags {
    Name        = "${var.stack_name}-dev-mysql-elb"
    managed_by  = "Stakater"
  }
...
resource "aws_elb" "mysql-elb-internal" {
  ...
  tags {
    Name        = "${var.stack_name}-dev-mysql-internal"
    managed_by  = "Stakater"
  }
...
module "mysql-scale-up-policy" {
  ...
  # Resource tags
  name = "${var.stack_name}-dev-mysql-scaleup-policy"
...
module "mysql-scale-down-policy" {
  ...
  # Resource tags
  name = "${var.stack_name}-dev-mysql-scaledown-policy"
